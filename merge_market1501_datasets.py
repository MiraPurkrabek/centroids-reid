...
def merge_datasets(
    ...
    sequence_number = 0
    for sequence_file in os.listdir(input_folder):
        if sequence_file.endswith(".{}".format(input_format)):
            
            sequence_folder = sequence_file.replace(".{}".format(input_format), "")
            if not os.path.isdir(os.path.join(input_folder, sequence_folder)):
                shutil.unpack_archive(os.path.join(input_folder, sequence_file), os.path.join(input_folder, sequence_folder), input_format)

            for subfolder in [TRAIN_DIR_NAME, TEST_DIR_NAME, QUERY_DIR_NAME]:
                for image_name in os.listdir(os.path.join(input_folder, sequence_folder, subfolder)):
                    print(image_name)
